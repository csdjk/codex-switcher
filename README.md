...
- **Windows 额度圆环**：任务栏和通知区域显示细圆环与居中数字，额度越少亮弧越短，并按绿／黄／红切换状态。无需打开主窗口即可查看剩余额度。
...
任务栏和通知区域的图标会随当前账号的剩余额度更新。圆环从顶部开始顺时针显示，中心数字表示剩余百分比；圆角端点、平滑字体和轻微柔光针对小尺寸显示做了优化。

<p align="center">
  <a href="docs/截图/额度圆环.png"><img src="docs/截图/额度圆环.png" alt="Windows 额度圆环：深浅背景下的充足、留意、低额度、用尽和未知状态" width="900"></a>
</p>
...
多个额度窗口同时存在时，显示**剩余最少的一项**。悬停通知区域图标可查看具体额度；隐藏主窗口后，后台仍会刷新。该圆环适用于正在运行的 Windows 桌面程序，未启动的固定快捷方式不会动态更新。

<p align="center">
  <a href="docs/截图/额度圆环实机.png"><img src="docs/截图/额度圆环实机.png" alt="额度圆环在 Windows 任务栏和通知区域中的实际显示" width="760"></a>
</p>
<p align="center"><sub>实际桌面截图，仅保留本应用图标；上方状态展示图使用程序渲染的测试额度。</sub></p>
...
按设备选择发布页中的对应文件，文件名中的 `<版本>` 为实际发行版本：

| 平台 | 推荐下载 |
| --- | --- |
| Windows x64 | `Codex-Switcher-v<版本>-Windows-x64-setup.exe`；另提供 `.msi` |
| macOS Apple Silicon | `Codex-Switcher-v<版本>-macOS-arm64.dmg` |
| macOS Intel | `Codex-Switcher-v<版本>-macOS-x64.dmg` |
| Linux x64 | `Codex-Switcher-v<版本>-Linux-x64.AppImage`；另提供 `.deb` 和 `.rpm` |

安装前请退出正在运行的 Codex Switcher。macOS 包暂未使用 Apple Developer ID 签名或公证；请核对下载来源，不要关闭系统整体安全检查。

### 自动更新

桌面应用启动后会检查本仓库 `csdjk/codex-switcher` 的最新正式发行版；发现版本号更高的版本时，显示更新说明和“前往下载”按钮，由用户打开本仓库的 [Releases](https://github.com/csdjk/codex-switcher/releases) 页面下载安装。浏览器／局域网模式不检查桌面程序更新。

当前只提供**检查新版本 → 前往下载 → 手动安装**，未启用应用内自动下载安装，也不会回退到原项目更新源。断网、超时或 GitHub API 限流时会跳过本次检查；这不代表已经是最新版，可直接访问上方发布页。

<p align="center">
  <a href="docs/截图/更新提示.png"><img src="docs/截图/更新提示.png" alt="更新提示明确显示 csdjk/codex-switcher 来源，并提供稍后和前往下载按钮" width="516"></a>
</p>
<p align="center"><sub>更新提示示例：使用隔离测试数据展示，不代表当前一定存在待安装更新。</sub></p>

更新源与签名配置的开发说明见 [自动更新来源与签名](docs/自动更新来源与签名.md)。
